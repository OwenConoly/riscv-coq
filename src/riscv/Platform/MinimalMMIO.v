...
Search Monad.
Check option_Monad.(left_identity).
...
Section Riscv.
  ...
  Definition setReg(reg: Z)(v: word)(regs: Registers): Registers :=
    if ((0 <? reg) && (reg <? 32)) then map.put regs reg v else regs. Check @leakage_of_instr. Print FreeMonad.free. Print free.
  Print riscv_primitive. Print leakage_of_instr. Check (fun mach => getReg (mach.(getRegs))).
